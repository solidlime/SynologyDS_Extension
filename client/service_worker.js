...
async function enqueueTask(uri, destination) {
...
var MENU_LINK = "ds-send-link";
var MENU_PAGE = "ds-send-page";
...
chrome.runtime.onMessage.addListener((msg, _sender, sendResponse) => {
  if (msg?.type === "FLUSH_QUEUE") {
    flushQueue().then((result) => sendResponse(result)).catch((err) => sendResponse({ error: err.message }));
    return true;
  }
});
...
async function trySend(url, destination) {
  ...
  await synoAPI.createTask(url, destination || settings.defaultDestination || void 0);
  return true;
}
async function flushQueue() {
  ...
  if (sent > 0) {
    ...
    const msg = remaining > 0 ? `Sent ${sent} queued task${sent !== 1 ? "s" : ""}. ${remaining} remaining.` : `All ${sent} queued task${sent !== 1 ? "s" : ""} sent successfully.`;
...
chrome.contextMenus.onClicked.addListener(async (info, tab) => {
  ...
  try {
    await trySend(url, settings.defaultDestination || void 0);
    ...
    if (!synoAPI.isLoggedIn) {
      try {
        ...
        await synoAPI.createTask(url, s.defaultDestination || void 0);
        notify("Task added", clip(url, 80));
        return;
      } catch {
      }
    }
    const queued = await enqueueTask(url, settings.defaultDestination || void 0);
    const queue = await loadQueue();
    notify(
      "Queued (offline)",
